  ...
  return (
    <div className="font-body max-w-[1100px]">
      ...
      <div className="bg-white rounded-2xl border border-navy/[0.07] shadow-sm overflow-hidden">
        {/* Header */}
        <div className="grid grid-cols-[1fr_120px_160px_80px] px-6 py-3.5 bg-navy/[0.03] border-b border-navy/[0.07]">
        ...
        {loading ? (
          ...
          users.map((u, i) => {
            ...
            return (
              <div
                key={u.id}
                className={`grid grid-cols-[1fr_120px_160px_80px] px-6 py-4 items-center hover:bg-navy/[0.02] transition-colors ${
                  i < users.length - 1 ? "border-b border-navy/[0.06]" : ""
                }`}
              >
                {/* User info */}
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    {isCurrentUser && (
                      <span className="text-[11px] text-blue font-medium">You</span>
                    )}
                  </div>
                </div>
                  ...
                  {isCurrentUser ? (
                    ...
                    <div className="flex gap-1.5">
                      <button
                        ...
                        className="h-7 px-2.5 rounded-lg bg-red-500 text-white text-xs font-semibold disabled:opacity-50"
                      >
                        {deleting === u.id ? "..." : "Confirm"}
                      </button>
                      <button
                        onClick={() => setDeleteConfirm(null)}
                        className="h-7 px-2 rounded-lg bg-navy/[0.06] text-navy/50 text-xs"
                      >
                        Cancel
                      </button>
                    </div>
