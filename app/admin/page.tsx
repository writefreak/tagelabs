...
type Contact = {
  ...
  subject: string; read: boolean; created_at: string;
};
type Project = {
...
export default function AdminDashboard() {
  ...
  const stats = [
    {
      ...
      change: `${published} published`, up: null,
    ...
    },
    {
      ...
      change: `${projects.length - published} draft${projects.length - published !== 1 ? "s" : ""} pending`,
      up: null,
  ...
  return (
    <div className="font-body max-w-[1100px]">
      {/* <div className="mb-8">
        <h2 className="font-display font-bold text-2xl text-navy">Hello, Welcome TageLabs 👋</h2>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mb-9">
        {stats.map((stat) => (
          <div key={stat.label} className="bg-white rounded-2xl p-6 border border-navy/[0.07] shadow-sm hover:-translate-y-0.5 hover:shadow-md transition-all duration-200">
            ...
            <p className={`text-xs mt-2.5 font-medium ${
              stat.up === false ? "text-red-400" : "text-navy/40"
            }`}>
              {stat.change}
            </p>
          </div>
